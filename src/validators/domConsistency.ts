 ...
 */
export function verifyDomConsistency(jsonLd: any, html: string): DomConsistencyCheckResult {
  const visibleText = extractTextFromHtml(html);
  const normalizedDom = normalizeText(visibleText);

  const checks: DomConsistencyCheckResult['checks'] = [];
  let passedScore = 0;
  let totalWeight = 0;
  ...
  const prod = graph.find(
    (item: any) => item['@type'] === 'Product' || item['@type'] === 'SoftwareApplication'
  );
  if (prod && prod.name) {
    totalWeight += 25;
  ...
  if (prod && prod.offers) {
    ...
    for (let i = 0; i < offers.length; i++) {
      ...
      if (offer.price !== undefined) {
        ...
        const match = priceVariations.some((variant) =>
          normalizedDom.includes(normalizeText(variant))
        );
