...
function parseTargetIdSelector(selector: string): string {
  const id = selector.startsWith('#') ? selector.slice(1) : selector;
  if (!SAFE_TARGET_ID.test(id)) {
    throw new MachineTrustConfigError(
      'INVALID_TARGET_SELECTOR',
      'targetSelector must be a safe HTML id or #id containing only letters, numbers, underscores, and hyphens.'
    );
  }
  return id;
}
