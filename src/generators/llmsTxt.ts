import type { LlmsTxtConfig, LlmsSection, LlmsLink } from '../types.js';
...
export function generateLlmsTxt(config: LlmsTxtConfig): string {
  ...
  if (config.tokenBudget && config.tokenBudget > 0) {
    ...
    if (currentTokens > config.tokenBudget) {
      ...
      console.warn(
        `[MachineTrust:llms.txt] Generated tokens (${currentTokens}) exceed tokenBudget (${config.tokenBudget})`
      );
    }
  }
...
export function generateLlmsFullTxt(config: LlmsTxtConfig): string {
  ...
  const lines: string[] = [baseLlms, '---', '', '## Comprehensive Documentation & Machine Index', ''];
...
export function parseLlmsTxt(content: string): ParsedLlmsTxt {
  ...
  const sections: ParsedLlmsTxt['sections'] = [];

  let currentSection: { title: string; description?: string; links: LlmsLink[] } | null = null;
