 ...
 * reference instead.
 */
export function createCanonicalNymrelOrganization() {
    return {
...
function assertPlainObject(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
        throw new MachineTrustConfigError('PARENT_ORGANIZATION_MALFORMED', "'parentOrganization' must be an object with non-empty string 'name' and 'url'");
    }
}
/**
 ...
 */
export function validateExplicitEntityRelationships(entity) {
    if (entity.nymrelAttribution !== undefined && typeof entity.nymrelAttribution !== 'boolean') {
        throw new MachineTrustConfigError('INVALID_NYMREL_ATTRIBUTION', `'entity.nymrelAttribution' must be a boolean when present (received ${typeof entity.nymrelAttribution})`);
    }
    ...
    if (entity.nymrelAttribution === true) {
        ...
        if (host === 'nymrel.com' || host.endsWith('.nymrel.com')) {
            throw new MachineTrustConfigError('CONTRADICTORY_NYMREL_ATTRIBUTION', `'entity.nymrelAttribution' contradicts the entity itself being a nymrel.com property (${entity.url}); owned properties are the attribution target, not its subject`);
        }
    }
    const seen = new Set();
    ...
    while (node !== undefined && node !== null) {
        ...
        if (typeof node.name !== 'string' || node.name.trim() === '') {
            throw new MachineTrustConfigError('PARENT_ORGANIZATION_MALFORMED', `'parentOrganization.name' must be a non-empty string at chain depth ${depth}`);
        }
        if (typeof node.url !== 'string' || node.url.trim() === '') {
            throw new MachineTrustConfigError('PARENT_ORGANIZATION_MALFORMED', `'parentOrganization.url' must be a non-empty string at chain depth ${depth} ('${node.name}')`);
        }
        const identity = `${node.name.trim()}|${node.url.trim()}`;
        if (seen.has(identity)) {
            throw new MachineTrustConfigError('PARENT_ORGANIZATION_CYCLE', `Explicit parentOrganization chain contains a cycle at '${node.name}'`);
        }
        seen.add(identity);
 ...
 */
function assertTruthfulParentIdentity(parentName, depth) {
    if (parentName === CANONICAL_NYMREL_ORG_NAME) {
        throw new MachineTrustConfigError('PARENT_ORGANIZATION_MISSTATEMENT', `'${CANONICAL_NYMREL_ORG_NAME}' cannot be declared as a corporate parent (chain depth ${depth}): it is the studio brand that BUILT this property. For built/partner attribution set entity.nymrelAttribution=true, which emits creator {"@id":"${NYMREL_ORGANIZATION_ID}"} by reference`);
    }
    if (parentName === CANONICAL_LEGAL_NAME) {
        throw new MachineTrustConfigError('PARENT_ORGANIZATION_MISSTATEMENT', `'${CANONICAL_LEGAL_NAME}' cannot be declared as a separate corporate parent (chain depth ${depth}): it is already the legalName of the '${CANONICAL_NYMREL_ORG_NAME}' organization itself; minting it as a parent node asserts a false subsidiary chain. On Nymrel-operated properties set entity.legalName='${CANONICAL_LEGAL_NAME}' and omit parentOrganization`);
    }
}
/**
...
export function validateNymrelLineage(jsonLd) {
    ...
    let graphNodes = [jsonLd];
    let org;
    if (Array.isArray(jsonLd['@graph'])) {
        graphNodes = jsonLd['@graph'];
        const orgs = graphNodes.filter((item) => isOrgNodeObject(item) && item['@type'] === 'Organization');
        org =
    ...
    const isSelfNymrel = trimmedOrgName(org) === CANONICAL_NYMREL_ORG_NAME ||
    ...
    const inlineCanonicalNode = graphNodes.find((item) => isOrgNodeObject(item) && item['@type'] === 'Organization' &&
        item['@id'] === NYMREL_ORGANIZATION_ID && item !== org);
    // Walk the declared parent chain (any depth), recording canonical identities.
    const parentChainCanonicalNames = [];
    ...
    const noRelationshipAtAll = !isSelfNymrel && !hasCreator && parentChainCanonicalNames.length === 0;
    ...
    if (isSelfNymrel) {
        ...
        if (legalName === undefined) {
            add('MISSING_LEGAL_NAME', 'error', `The '${CANONICAL_NYMREL_ORG_NAME}' organization must carry legalName '${CANONICAL_LEGAL_NAME}' so machines can verify the operating legal entity`);
        }
        else if (legalName !== CANONICAL_LEGAL_NAME) {
            add('INCORRECT_LEGAL_NAME', 'error', `The '${CANONICAL_NYMREL_ORG_NAME}' organization legalName ${JSON.stringify(org.legalName)} does not match canonical '${CANONICAL_LEGAL_NAME}'`);
        }
        if (org.parentOrganization !== undefined && org.parentOrganization !== null) {
            add('FALSE_SUBSIDIARY_CHAIN', 'error', `'${CANONICAL_NYMREL_ORG_NAME}' declares a parentOrganization but has none: it is not a subsidiary. Express the legal entity as legalName '${CANONICAL_LEGAL_NAME}' on this same node and remove the parentOrganization chain`);
        }
    }
    // --- Attribution-by-reference rules (built/partner properties).
    if (hasCreator) {
        const creator = org.creator;
        const validReference = isOrgNodeObject(creator) &&
            Object.keys(creator).length > 0 &&
            creator['@id'] === NYMREL_ORGANIZATION_ID &&
            Object.keys(creator).every((key) => key === '@id' || key === '@type');
        if (!validReference) {
            add('MALFORMED_CREATOR_REFERENCE', 'error', `creator must be the bare reference {"@id":"${NYMREL_ORGANIZATION_ID}"}; rebuilt or altered creator nodes misstate the relationship`);
        }
        if (inlineCanonicalNode) {
            add('CANONICAL_NODE_REBUILT', 'error', `Graph embeds a full Organization node for ${NYMREL_ORGANIZATION_ID} alongside a creator reference; attribute by @id only and do not rebuild the canonical organization inline`);
        }
    }
    ...
    for (const name of parentChainCanonicalNames) {
        if (name === CANONICAL_NYMREL_ORG_NAME && !isSelfNymrel) {
            add('PARENT_MISSTATEMENT', 'error', `'${CANONICAL_NYMREL_ORG_NAME}' is listed as a corporate parent but it is the studio that BUILT this property. Use creator {"@id":"${NYMREL_ORGANIZATION_ID}"} (entity.nymrelAttribution=true) instead of a parentOrganization chain`);
        }
        if (name === CANONICAL_LEGAL_NAME && !isSelfNymrel) {
            add('PARENT_MISSTATEMENT', 'error', `'${CANONICAL_LEGAL_NAME}' is listed as a separate corporate parent but it is already the legalName of the '${CANONICAL_NYMREL_ORG_NAME}' organization itself; minting it as a parent node asserts a false subsidiary chain`);
        }
    }
    return finalizeLineageResult(issues);
}
